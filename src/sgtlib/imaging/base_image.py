...
class BaseImage:
    ...
    @staticmethod
    def compute_fractal_dimension(cv_img_bin: np.ndarray) -> float:
        """
        Calculate the fractal dimension of a 2D image using the box-counting method.
        Compute fractal dimension of a binary or grayscale image using a  box-counting method similar to ImageJ's
        FracLac plugin.

        Parameters:
            cv_img_bin (np.ndarray): Input grayscale image.
        ...
        # Ensure the image is bin_img
        bin_img = cv_img_bin.copy()
        if len(cv_img_bin.shape) == 3:
            cv_img_bin = cv2.cvtColor(cv_img_bin, cv2.COLOR_BGR2GRAY)
            _, bin_img = cv2.threshold(cv_img_bin, 128, 255, cv2.THRESH_BINARY)

        # Invert so non-empty pixels are 1
        bin_img = bin_img > 0
        ...
        n_offsets = 10
        min_box_size = 2
        max_box_size = min(bin_img.shape) // 2
        # Loop over box sizes (powers of two like FracLac)
        box_sizes = np.array([2 ** i for i in range(int(np.log2(min_box_size)), int(np.log2(max_box_size)) + 1)])
        for size in box_sizes:
            ...
            for _ in range(n_offsets):
                ...
                region = bin_img[offset_y:, offset_x:]
                ...
                reshaped = region[:num_h * size, :num_w * size].reshape(num_h, size, num_w, size)
                boxes = reshaped.any(axis=(1, 3))  # True if any pixel in the box is a foreground
                total_count += np.sum(boxes)

            avg_count = total_count / n_offsets
            sizes.append(size)
            counts.append(avg_count)

        # Log-log fit to estimate slope
        coeffs = scipy.stats.linregress(np.log(1 / np.array(sizes)), np.log(counts))
        return coeffs.slope  # This is the fractal dimension estimate

    @staticmethod
    def compute_fractal_dimension_imagej(cv_img_bin: np.ndarray) -> None|float:
        """
        Compute fractal dimension using ImageJ's FracLac plugin via PyImageJ.

        Parameters:
            cv_img_bin (np.ndarray): Grayscale or binary image (2D, uint8)

        Returns:
            float: Estimated fractal dimension, or None if it fails.
        """
        import imagej
        ...
        # Convert NumPy image (CV2) to ImageJ ImagePlus
        # PyImageJ expects Java images; convert via numpy array bridge
        ij_img = ij.py.to_java(cv_img_bin)

        # Display isn't needed, so skip ij.ui().show()

        # Run the Fractal Box Count (FracLac) plugin
        # Note: the exact plugin command may vary depending on installation
        try:
            # Example command; refer to ImageJ macro plugin naming if needed
            result = ij.op().run("fraclac.boxCount", ij_img)
        except Exception as e:
            print(f"Failed to run FracLac Box Count: {e}")
            return None

        # The plugin result is often a dataset or result table
        # Retrieve *Fractal Dimension* from the results
        # This retrieval depends on how the plugin returns values.
        # For illustration only:
        try:
            fd = float(result.get("Fractal Dimension"))
            return fd
        except Exception as e:
            print(f"Error extracting fractal dimension: {e}")
            return None
